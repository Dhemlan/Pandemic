...
{   
    ...
    public IEnumerator playerDraw(Player curPlayer, PlayerCard card1, PlayerCard card2){
        ...
        yield return StartCoroutine(moveCardToCentre(playerCard1.transform, mapCentreLeft, playerCardFaces[(int)card1.getColour()], new Vector3(1.5f,1.5f,1.5f)));
        playerCard1Title.text = card1.getName();
        yield return StartCoroutine(moveCardToCentre(playerCard2.transform, mapCentreRight, playerCardFaces[(int)card2.getColour()], new Vector3(1.5f,1.5f,1.5f)));
        ...
        if (card1.getName().Equals("Epidemic")){
            ...
            yield return StartCoroutine(moveToDiscard(playerCard1.transform, handCentres[curPlayer.getTurnOrderPos() - 1]));
        }
        if (card2.getName().Equals("Epidemic")){
            yield return StartCoroutine(moveToDiscard(playerCard2.transform, playerDiscardCentre));
        }
        else {
            yield return StartCoroutine(moveToDiscard(playerCard2.transform, handCentres[curPlayer.getTurnOrderPos() - 1]));
        }
